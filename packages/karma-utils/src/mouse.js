...
/**
 * @param {Array<{type: string, x: number, y: number, options: Object}>} events List of events.
 * @return {Array<{type: string, x: number, y: number, options: Object}>} The cleaned up array that can be accepted by the Puppeteer.
 */
function cleanupMouseEvents(events) {
  const { x: offsetX, y: offsetY } = window.frameElement
    ? window.frameElement.getBoundingClientRect()
    : { x: 0, y: 0 };
  return events.map(({ type, x, y, options }) => {
    const xy =
