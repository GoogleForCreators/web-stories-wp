...
export default function useSettingsApi() {
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      dispatch({
        type: ActionType.FetchSettingsSuccess,
        payload: response,
      });
    } catch (err) {
      dispatch({
        type: ActionType.FetchSettingsFailure,
        payload: {
  ...
  const updateSettings = useCallback(
    async (queryParams) => {
      dispatch({ type: ActionType.SettingSaved, payload: false });
      try {
        ...
        dispatch({ type: ActionType.SettingSaved, payload: true });
      } catch (err) {
        dispatch({
          type: ActionType.UpdateSettingsFailure,
          payload: {
