...
export default function useFontsApi() {
  ...
  const [customFonts, setCustomFonts] = useState<WordPressFont[]>([]);

  const fetchCustomFonts = useCallback(async (): Promise<
    WordPressFont[] | null
  > => {
    try {
  ...
  const deleteCustomFont = useCallback(
    async (id: number): Promise<WordPressFont> => {
  ...
  const addCustomFont = useCallback(
    async (font: Font): Promise<WordPressFont> => {
