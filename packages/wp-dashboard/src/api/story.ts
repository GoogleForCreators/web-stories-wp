 ...
 */
export function fetchStories(config: WPDashboardConfig, queryParams) {
  const {
    page = 1,
  ...
  return apiFetch<EnvelopedWordPressStories>({
    path: addQueryArgs(config.api.stories, query),
  }).then(({ body: stories, headers }) => {
    const totalPages = headers ? parseInt(headers['X-WP-TotalPages']) : 0;
    const totalStoriesByStatus = headers
      ? JSON.parse(headers['X-WP-TotalByStatus'])
      : {};

    const fetchedStoryIds: number[] = [];
 ...
 * @return Request promise.
 */
export function trashStory(config: WPDashboardConfig, storyId: number) {
...
export function updateStory(
  ...
  return apiFetch({
    ...
    data,
    method: 'POST',
  }).then((response: WordPressStory) => reshapeStoryObject(response));
}
...
export function createStoryFromTemplate(
  ...
  return apiFetch<WordPressStory>({
    path,
    data: {
      ...storyPropsToSave,
...
export function duplicateStory(
  ...
  const path = addQueryArgs(config.api.stories, {
    _embed: STORY_EMBED,
    _fields: STORY_FIELDS as string[],
  });
