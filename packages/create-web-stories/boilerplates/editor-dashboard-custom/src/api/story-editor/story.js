...
export const saveStoryById = ({
  ...
  const storyData = JSON.parse(window.localStorage.getItem(LOCAL_STORAGE_CONTENT_KEY)) || {};
  if(storyData?.storySaveData?.storyId){
    storyData[storySaveData?.storyId] = storySaveData;
  }else{
    storyData[storySaveData?.storyId] = { ...storySaveData, created: new Date().toISOString(), createdGmt: new Date().toISOString() };
  }
