...
until $(curl -L http://$WP_HOST:$HOST_PORT -so - 2>&1 | grep -q "WordPress"); do
	echo -n '.'
	sleep 5
done
echo ''
...
wp core install --title="$SITE_TITLE" --admin_user=admin --admin_password=password --admin_email=test@test.com --skip-email --url=http://$WP_WP_HOSTHOST:$HOST_PORT --quiet
...
if [ "$CURRENT_URL" != "http://$WP_HOST:$HOST_PORT" ]; then
	wp option update home "http://$WP_HOST:$HOST_PORT" --quiet
	wp option update siteurl "http://$WP_HOST:$HOST_PORT" --quiet
fi
