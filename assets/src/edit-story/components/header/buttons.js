...
function PreviewButton() {
  const { isSaving, link, status, autoSave, saveStory } = useStory((state) => {
    const {
      state: {
    ...
    return { isSaving, link, status, autoSave, saveStory };
  });
...
function Publish() {
  const { isSaving, date, storyId, saveStory } = useStory((state) => {
    const {
      state: {
        // eslint-disable-next-line no-shadow
        meta: { isSaving },
    ...
    return { isSaving, date, storyId, saveStory };
  });
...
function SwitchToDraft() {
  const { isSaving, saveStory } = useStory((state) => {
    const {
      state: {
        // eslint-disable-next-line no-shadow
        meta: { isSaving },
...
function Update() {
  const { isSaving, status, saveStory } = useStory((state) => {
    const {
      state: {
        // eslint-disable-next-line no-shadow
        meta: { isSaving },
    ...
    return { isSaving, status, saveStory };
  });
  const { isUploading } = useMedia((state) => ({
    isUploading: state.state.isUploading,
  }));
