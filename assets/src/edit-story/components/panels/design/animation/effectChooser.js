...
const GridItem = styled.button.attrs({ role: 'listitem' })`
  ...
  background: ${({ active, theme }) =>
    active ? theme.old.colors.accent.primary : '#333'};
  border-radius: 4px;
  ...
  &:focus:not([aria-disabled='true']) {
    background: ${({ active, theme }) =>
      active ? theme.old.colors.accent.primary : '#1C73E8'};
...
const NoEffect = styled(GridItemFullRow)`
  ${({ theme }) => css`
    ...
    font-family: ${theme.old.fonts.paragraph.small.family};
    font-size: ${theme.old.fonts.paragraph.small.size};
    line-height: ${theme.old.fonts.paragraph.small.lineHeight};
    font-weight: normal;
  `}
`;
