...
function useScroll(scrollTarget, isReordering) {
  ...
  const updateScrollMarkers = useBatchingCallback(() => {
    ...
    if (possibleScroll <= 0) {
      ...
      setHasScrollAbove(scrollTarget.scrollTop > 0);
      setHasScrollBelow(scrollTarget.scrollTop < possibleScroll);
    }
  }, [scrollTarget]);
  ...
  useEffect(() => {
    ...
    let mounted = true;
    const update = () => {
      scrollTarget.scrollTop += scrollDirection * SCROLL_DISTANCE;
      updateScrollMarkers();
      if (mounted) {
