 ...
 */
import { getAMPValidationErrors, WrapElement } from './utils';

async function toBeValidAMPStoryElement(stringOrComponent) {
  const string = renderToStaticMarkup(stringOrComponent);
  const errors = await getAMPValidationErrors(
    <WrapElement>{stringOrComponent}</WrapElement>
  );
