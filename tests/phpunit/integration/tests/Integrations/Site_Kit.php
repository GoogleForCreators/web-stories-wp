...
class Site_Kit extends DependencyInjectedTestCase {
	 ...
	 * @covers ::register
	 */
	public function test_analytics_tracking_handler_webstories(): void {
		// Init Analytics Module.
		\define( 'GOOGLESITEKIT_VERSION', '1.2.3' );

		// Update flexible switching option.
		update_option( 'web_stories_ga_tracking_handler', 'web-stories', false );

		// Get Analytics Module.
		$analytics = $this->container->get( 'analytics' );
		add_action( 'web_stories_print_analytics', array( $analytics, 'print_analytics_tag' ) );

		// Register Site_Kit integration.
		$this->instance->register();

		// Check for filters & actions.
		$this->assertSame(
			10,
			has_filter(
				'googlesitekit_analytics-4_tag_amp_blocked',
				function ( $blocked ) {
					if ( $this->instance->context->is_web_story() ) {
						return true;
					}
					return $blocked;
				}
		...
		$this->assertSame( 10, has_action( 'web_stories_print_analytics', array( $analytics, 'print_analytics_tag' ) ) );
	}

	/**
	 * @covers ::register
	 */
	public function test_analytics_tracking_handler_site_kit(): void {
		// Init Analytics Module.
		\define( 'GOOGLESITEKIT_VERSION', '1.2.3' );

		// Update flexible switching option.
		update_option( 'web_stories_ga_tracking_handler', 'site-kit', false );
		...
		$this->assertFalse( has_action( 'web_stories_print_analytics', array( $analytics, 'print_analytics_tag' ) ) );
	}

	/**
	 * @covers ::register
	 */
	public function test_analytics_tracking_handler_both(): void {
		// Init Analytics Module.
		\define( 'GOOGLESITEKIT_VERSION', '1.2.3' );
